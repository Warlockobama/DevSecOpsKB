...
func TestNormalizeDefinitionOrigins_DefaultsScannerSourcesToToolUnlessProjectSpecific(t *testing.T) {
	ef := EntitiesFile{
		SourceTool: "zap",
		Definitions: []Definition{
			{DefinitionID: "def-10001", PluginID: "10001"},
	...
	if got := ef.Definitions[1].Origin; got != DefinitionOriginCustom {
		t.Fatalf("project-specific zap plugin origin = %q, want %q", got, DefinitionOriginCustom)
	}
	if got := ef.Definitions[2].Origin; got != DefinitionOriginCustom {
