...
func TestE2E_IngestVaultConfluenceDryRun(t *testing.T) {
	...
	// Step 4: assert triage-board.md open count == 2.
	// The fixture has 2 occurrences with nil analyst (occ-001, occ-002).
	// Triage board row format: | Open | <issueCount> | <occCount> |
...
// triageBoardOpenCount returns true when the triage-board markdown contains an
// "Open" status row whose occurrence column matches wantOccCount.
