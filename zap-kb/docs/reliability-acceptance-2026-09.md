...
| --- | --- | --- |
| G1 Jira and sink outcomes | Jira package `6dea1c9` and final CLI outcomes `050846a`; final local publisher-image contract passes Cloud create/readback, rejected-create and gateway routing | Designated Jira/Confluence tenant |
...
| G5 Configuration | `24ee34a`, `0803341`; explicit flag/environment/default, gateway and safe-error tests plus final container combination pass | Live destination acceptance remains external |
...
| G7 Maintainability and release | Provenance/CI `5313680`, `f4aae32`, `58e3aff`; pipeline extraction `38238f8`; writable state default `2b78dfc`; immutable demo assertion `ae80463`; all local final gates pass | Live destinations and large-wiki production acceptance require designated environments |
...
- At `2b78dfc`/`ae80463`, the final local publisher-image contract and disposable
  Forgejo replay passed. The latter verified byte-identical source input after
  publication, a separate journal event, identical replay without issue/wiki
  mutation, changed-run history, and preservation of the analyst-owned accepted
  label. The sanitized artifact hash was
...
The original CLI cases now run together in the combined suite. The final local
candidate passed the container and disposable Forgejo integrations after the
maintainability work, along with the publication-state paused-create regression,
full uncached Go suite, vet, build, focused race suites and companion Kubernetes
render. These local/disposable passes do not substitute for designated-tenant or
large production-wiki acceptance.
